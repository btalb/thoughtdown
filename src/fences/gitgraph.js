...
function createHash(length = DEFAULT_CONFIG.hash_length, seed = null) {
  ...
  return Math.floor(
    16 ** (length - 1) + Math.random() * (16 ** length - 16 ** (length - 1))
  ).toString(16);
}
          ...
          while (++ii < parsedGraph[i].length) {
            ...
            if (xx == '/') {
              ...
              if (/^[A-Za-z]+$/.test(parsedGraph[iii][ii])) {
                sources.push(
                  gg.nodes.findIndex(d => d.id === parsedGraph[iii][ii])
                );
              }
            }
          }
      ...
      if (n.hasOwnProperty('labels')) {
        ...
        n.labels.forEach(function (l, i) {
          gg.labels.push({
            ...
            dx: x(i),
            dy:
              n.y === 0 ? data.config.label_buffer : -data.config.label_buffer,
            d: n.y === 0 ? -1 : 1,
...
export default function generateHtml(dataString, configString) {
  ...
  let x = d3
    .scaleLinear()
    .domain(getLimits(ggData.nodes, n => n.x, data.config.min_columns))
    .range([
      data.config.margin_left,
      data.config.width - data.config.margin_right,
    ]);
  let y = d3
    .scaleLinear()
    .domain(getLimits(ggData.nodes, n => n.y, data.config.min_rows))
    .range([
      data.config.height - data.config.margin_bottom,
      data.config.margin_top,
    ]);
  ...
  let labelLink = function (l) {
    ...
    if (l.type === 'head') {
      p.moveTo(
        -data.config.label_width / 2,
        (-data.config.label_height * l.d) / 2
      );
      p.lineTo(
        -data.config.label_width / 2 -
          (data.config.label_spacing - data.config.label_width) +
          5,
        (-data.config.label_height * l.d) / 2
      );
    } else {
      ...
      p.quadraticCurveTo(
        0,
        -l.dy / 2,
        -l.dx - side * data.config.node_radius * 1.25,
        -l.dy - (l.d * data.config.node_radius) / 2
      );
    }
    return p.toString();
  };
  ...
  let svg = d3
    ...
    .attr('font-size', data.config.font_size);
  svg
    .append('rect')
    .attr('width', '100%')
    .attr('height', '100%')
    .attr('fill', data.config.color_background);
  ...
  svg
    .selectAll('gitgraph-link')
    .data(ggData.links)
    .enter()
    .append('path')
    .classed('gitgraph-link', true)
    .attr('fill', 'none')
    .attr('stroke', graphLinkColor)
    .attr('stroke-width', '5')
    .attr('d', graphLink);
  svg
    .selectAll('gitgraph-node')
    .data(ggData.nodes)
    .enter()
    .append('circle')
    .classed('gitgraph-node', true)
    .attr('fill', d => (data.config.show_head && d.head ? 'none' : d.color))
    .attr('stroke', d => d.color)
    .attr('stroke-width', 5)
    .attr('cx', d => x(d.x))
    .attr('cy', d => y(d.y))
    .attr('r', data.config.node_radius);
  if (data.config.show_hash || data.config.show_name) {
    svg
      .selectAll('gitgraph-node-label')
      .data(ggData.nodes)
      .enter()
      .append('text')
      .classed('gitgraph-node-label', true)
      .attr('x', d => x(d.x))
      .attr('y', d => y(d.y) - 1.25 * data.config.node_radius)
      .attr('text-anchor', 'middle')
      .attr('font-weight', 'bold')
      .attr('fill', d => d.color)
      .text(d => (data.config.show_name ? d.name : d.hash));
  }
  if (ggData.labels && ggData.labels.length) {
    let gs = svg
      .selectAll('gitgraph-label')
      .data(ggData.labels)
      .enter()
      .append('g')
      .classed('gitgraph-label', true)
      .attr(
        'transform',
        d =>
          'translate(' + (x(d.ref_x) + d.dx) + ',' + (y(d.ref_y) + d.dy) + ')'
      );
    gs.append('path')
      .attr('fill', 'none')
      .attr('stroke', 'black')
    ...
    gs.filter(d => d.type !== 'plain')
      .append('path')
      .attr('fill', d => data.config[`color_${d.type}`])
      .attr('stroke', 'black')
      .attr('stroke-width', '1')
      .attr('d', label);
    gs.append('text')
      ...
      .attr('y', d => (-d.d * data.config.label_height) / 2);
  }
